...
the approach to testing controllers that handle HTTP requests by using the Spring MVC com.Test
framework -- that is, without running a Servlet container but relying on the Spring Framework
to invoke the annotated controllers. As with Spring MVC com.Test, you have two
