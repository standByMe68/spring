...
The Spring MVC com.Test framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC com.Test Framework>> instead.
...
com.Test classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of com.Test Fixtures
...
com.Test property sources have higher precedence than those loaded from the operating
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@com.Test(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@com.Test(timeout=...)`
...
Since JUnit Jupiter supports the use of `@com.Test`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@com.Test` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@com.Test // org.junit.jupiter.api.com.Test
...
* com.Test instance post-processing.
...
for which it is responsible. com.Test instances do not automatically receive access to the
...
com.Test classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with com.Test Property Sources
...
====== Declaring com.Test Property Sources
...
====== Inheriting and Overriding com.Test Property Sources
...
.com.Test suites and forked processes
[NOTE]
====
...
===== com.Test-managed Transactions

com.Test-managed transactions are transactions that are managed declaratively by using the
...
hierarchy runs within a transaction. com.Test methods that are not annotated with
...
----
	...
	@Transactional
	@com.Test // no expected exception!
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@com.Test`, `@RepeatedTest`,
...
[[spring-mvc-test-framework]]
=== Spring MVC com.Test Framework

The Spring MVC com.Test framework provides first class support for testing Spring MVC code
...
Spring MVC com.Test also provides client-side support for testing code that uses the
...
The goal of Spring MVC com.Test is to provide an effective way to test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC com.Test builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
...
Jupiter-based example uses Spring MVC com.Test:
...
In Spring MVC com.Test, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC com.Test for container-less testing of streaming
...
===== Spring MVC com.Test vs End-to-End Tests

Spring MVC com.Test is built on Servlet API mock implementations from the
...
There are pros and cons for each approach. The options provided in Spring MVC com.Test are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC com.Test fall under the category of classic unit
...
Another important distinction when using Spring MVC com.Test is that, conceptually, such
...
sample tests] intended to show how to use Spring MVC com.Test. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full test coverage based on Spring MVC com.Test.
...
* com.Test JavaScript within pages.
...
With Spring MVC com.Test, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC com.Test framework, make sure that your project
...
----
	...
	// com.Test code that uses the above RestTemplate ...
...
Spring MVC com.Test's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/com.Test%20Spy.html[com.Test Spy] pattern.
